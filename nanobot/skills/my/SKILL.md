...
1. Identify the situation from the table below, call the `my` tool with the appropriate action.
2. If `set`, warn the user before changing impactful settings (model, iterations).
3. For detailed examples, read [references/examples.md](references/examples.md).
...
<rule>
**Only set when benefit is clear and user is informed.** Bias toward stability; only set when defaults are genuinely insufficient.
</rule>
...
- `model_preset` persists for the current session; other modifications are in-memory only
- Direct `model` / `context_window_tokens` writes are rejected during active sessions (they'd change the shared instance default) — use a configured `model_preset` instead
