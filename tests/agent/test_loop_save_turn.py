...
def test_save_turn_keeps_multimodal_runtime_context_for_model_replay() -> None:
    ...
    assert session.messages[0]["content"] == [{"type": "text", "text": "provider context"}]
...
async def test_internal_continuation_preserves_streaming_route_metadata(
    ...
    async def fake_run_agent_loop(
        initial_messages, *, on_stream=None, on_stream_end=None, **_kwargs
    ):
        nonlocal calls
        calls += 1
        if calls == 1:
...
async def test_startup_scan_reinjects_runtime_checkpoint_session(tmp_path: Path) -> None:
    ...
    assert any(
        m["role"] == "tool" and m.get("tool_call_id") == "call_pending" for m in session.messages
    )
...
async def test_startup_scan_replays_pending_injections(tmp_path: Path) -> None:
    ...
    kinds = [
        "resume" if turn_continuation.gateway_resume_inbound(m.metadata) else "other"
        for m in messages
    ]
    assert "resume" in kinds
    follow_up = next(
        m for m in messages if not turn_continuation.gateway_resume_inbound(m.metadata)
    )
