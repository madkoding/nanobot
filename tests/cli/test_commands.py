...
def test_plugins_list_uses_explicit_config(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        ...
        lambda enabled_names=None: (
            {"example": plugin} if enabled_names is None or "example" in enabled_names else {}
        ),
    )
    monkeypatch.setattr(
...
def test_provider_login_xai_grok_runs_browser_flow_with_configured_proxy(monkeypatch):
    ...
    assert (
        "Hosted X Search is enabled automatically when the selected model supports it"
        in result.stdout
    )
...
def test_webui_foreground_attaches_to_existing_managed_gateway(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("nanobot.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr(
        "nanobot.cli.commands._gateway_health_ready", lambda *_args, **_kwargs: True
    )
    monkeypatch.setattr(
        "nanobot.cli.commands._webui_endpoint_reachable", lambda *_args, **_kwargs: True
    )
    monkeypatch.setattr(
        "nanobot.cli.commands._open_webui_browser",
...
def test_webui_foreground_refuses_occupied_webui_port(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("nanobot.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr(
        "nanobot.cli.commands._gateway_health_ready", lambda *_args, **_kwargs: False
    )
    monkeypatch.setattr(
        "nanobot.cli.commands._webui_endpoint_reachable", lambda *_args, **_kwargs: True
    )
    monkeypatch.setattr(
        "nanobot.cli.commands._tcp_endpoint_reachable", lambda *_args, **_kwargs: False
    )
    monkeypatch.setattr(
        "nanobot.cli.commands._run_gateway",
...
def test_gateway_unbound_agent_cron_is_skipped(monkeypatch, tmp_path: Path) -> None:
    config_file = tmp_path / "instance" / "config.json"
    config_file.parent.mkdir(parents=True)
    config_file.write_text("{}")
...
def test_gateway_bound_cron_runs_as_session_turn(monkeypatch, tmp_path: Path) -> None:
    config_file = tmp_path / "instance" / "config.json"
    config_file.parent.mkdir(parents=True)
    config_file.write_text("{}")
